"""Brand Sentiment Score (BSS) — Command-Line Interface.
...
def main(verbose: bool) -> None:
    """Brand Sentiment Score (BSS) — predict brand popularity from free public data."""
...
def compare(brands: str, engine: str, output: str | None) -> None:
    ...
    if len(results) > 1:
        ...
        for i, r in enumerate(sorted_results, 1):
            click.echo(f"  {i}. {r.brand_name}: {r.bss:.1f}/100 ({r.grade()})")
...
@click.option("--engine", "-e", type=click.Choice(["vader", "textblob", "ensemble"]), default="vader")
def export(brand: str, ticker: str | None, fmt: str, output: str, engine: str) -> None:
